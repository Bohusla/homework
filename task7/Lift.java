...
// Не совсем верно написана логика в задании про лифт.
// Например, если я введу такие значения: количество этажей - 10, поднимаемся на 12 этажей, опускаемся на 5 этажей, 
// то программа выдаст, что количество подъёмов - 2. Хотя на самом деле должно быть 1.
// Внутри цикла следует сначала поднимать лифт, затем проверять, не достигли ли мы верха, 
// и только потом опускать лифт. Вот такая логика будет корректна.
// Методы следует называть с маленькой буквы.
// Также внутри класса необходимо соблюдать порядок расположения его членов - сначала поля, затем конструкторы, затем методы.
